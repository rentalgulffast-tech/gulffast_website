...
const POINTS: ChecklistPoint[] = [
  ...
  { title: 'HSE-First Approach', text: 'Equipment and mobilization procedures prepared to meet standard industrial site safety and gate-access requirements.' },
...
export default function WhyChooseUsChecklist() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
      {POINTS.map((point) => (
        <div key={point.title} className="flex items-start gap-3 bg-white border border-[#D7E6F5] rounded-2xl p-5 shadow-sm">
          <span className="shrink-0 w-9 h-9 rounded-xl bg-[#EAF4FC] border border-[#D7E6F5] text-[#2B6CB0] flex items-center justify-center">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
            </svg>
          </span>
          <div>
            <h3 className="font-bold text-[#12233B] text-sm mb-1">{point.title}</h3>
