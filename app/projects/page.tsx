  ...
  return (
    <div className="py-10 bg-background text-foreground min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white border border-border rounded-2xl p-5 mb-8 flex items-start gap-4 text-xs text-slate-700 shadow-sm">
          <span className="w-8 h-8 rounded-xl bg-accent-strong/10 text-accent-strong flex items-center justify-center font-bold text-base shrink-0 border border-accent-strong/20">ℹ️</span>
          <div>
            <strong className="text-primary block mb-1">Direct Supplier Confidentiality Notice</strong>
            <span>
        ...
        <div className="space-y-6 my-8">
          {caseStudies.map((cs) => (
            <div key={cs.id} className="bg-white border border-border rounded-2xl p-6 sm:p-8 shadow-sm space-y-5">
              
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-border pb-4">
                <div>
                  <span className="text-xs text-accent-strong font-bold uppercase tracking-wider">{cs.sector}</span>
                  <h2 className="text-2xl font-extrabold text-primary mt-1">{cs.title}</h2>
                </div>
                <div className="text-xs text-primary bg-background px-3 py-1.5 rounded-xl border border-border shrink-0 font-bold">
                  📍 {cs.location}
                </div>
              </div>
              ...
              <div className="bg-tint rounded-xl p-5 border border-border space-y-3">
                <h3 className="text-xs font-bold text-primary uppercase tracking-wider">Deployment Highlights:</h3>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2 text-xs">
                ...
                <Link
                  ...
                  className="px-5 py-2.5 bg-primary hover:bg-accent-strong text-white font-bold rounded-xl transition-colors inline-block text-center shadow-sm"
                >
