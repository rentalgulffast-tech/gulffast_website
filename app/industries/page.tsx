      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-8 my-8">
          {industries.map((industry, idx) => (
            <div key={industry.slug} className="bg-white border border-[#D7E6F5] rounded-2xl p-8 shadow-sm grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              <div className="lg:col-span-8 space-y-3">
                <span className="text-[#2B6CB0] font-bold text-xs uppercase tracking-wider">
                  {String(idx + 1).padStart(2, '0')}. Sector Focus
                </span>
                <h2 className="text-2xl sm:text-3xl font-extrabold text-[#12233B]">
                  {industry.name}
                </h2>
              ...
              <div className="lg:col-span-4 bg-[#EAF4FC] p-6 rounded-xl border border-[#D7E6F5] text-center space-y-3">
                <h3 className="text-base font-bold text-[#12233B]">{industry.tagline}</h3>
                <Link href={`/industries/${industry.slug}`} className="block w-full py-2.5 bg-[#2B6CB0] hover:bg-[#1D6FB8] text-white font-bold text-xs rounded-xl transition-colors shadow-sm">
